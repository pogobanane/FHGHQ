...
//////////////////////////////////////////////
class TopTowns extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (
      JSON.stringify(this.props.timelinesnap.spec.toptowns) !=
      JSON.stringify(nextProps.timelinesnap.spec.toptowns)
    ) {
  ...
  render() {
    ...
    for (let town in this.props.timelinesnap.spec.toptowns) {
      ...
      toptowns.push({
        name: U.GetTownName(obj.rId, obj, this.props.startpoint.static),
        amount: obj.amount,
        side: obj.side,
      });
    }
    ...
    for (let i = 0; i < toptowns.length; i++) {
      ...
      if (toptowns[i].side == 1) {
        sideimg = (
          <img
            className='stats_toptowns_sideimg'
      ...
      toptowns[i] = (
        <tr key={'stats_toptowns_tr_' + toptowns[i].name} className={trclass}>
          <td>{sideimg}</td>
          <td className='stats_toptowns_nametd'>{toptowns[i].name}</td>
          <td>
            <AnimatedNumber
              value={toptowns[i].amount}
              duration={600}
              stepPrecision={0}
            />
          </td>
    ...
    return (
      <div id='stats_toptowns' className='stats_div'>
