...
class ChatCore extends React.Component {
  render() {
    if (this.props.tab.tab != 7) {
      ...
      return (
        <div className='row' id='chat_totalcontainer'>
          <div
            className='col chat_window_container'
            id='chat_window_container_general'
          >
          ...
          <div
            className='col chat_window_container'
            id='chat_window_container_announcements'
          >
...
class Chat extends React.Component {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <div className='chat_window' id={'chat_window_' + this.props.category}>
          <ul>{messages}</ul>
        </div>
        {this.props.category != 'announcements' ||
        this.props.users.myrank < 3 ? (
          <ChatInput category={this.props.category} />
        ) : null}
      </React.Fragment>
    );
  }
...
class Message extends React.Component {
  render() {
    ...
    return (
      <li className='chat_message_li'>
        <p className='chat_message_meta'>
          <img className='chat_userimg' src={user.avatar} />
          <span className={'chat_message_username ' + username_class}>
            {user.name}
          </span>{' '}
          <span className='chat_message_datespan'>
            {new Date(message.date).toLocaleDateString()}{' '}
            {new Date(message.date).toLocaleTimeString()}
          </span>
        </p>
        <p className={message_class}>{message.message}</p>
      </li>
    );
  }
}
...
class ChatPersonal extends React.Component {
  ...
  render() {
    ...
    if (this.props.chat != undefined) {
      if (this.props.chat[category] != undefined) {
        for (let i = 0; i < this.props.chat[category].length; i++) {
          ...
          let user = U.GetUser(this.props.users.users, message.userid);
          messages.push(
            <Message user={user} message={message} category={category} />
          );
        }
      }
    }
    ...
    return (
      <div className='chat_window_container' id='chat_personal_totalcontainer'>
        <div className='chat_window_header' id='chat_window_header_personal'>
          <span className={'chat_message_username ' + username_class}>
            <img className='chat_userimg' src={user.avatar} />
            {user.name}
          </span>
        ...
        <div className='chat_window' id={'chat_window_personal'}>
          <ul>{messages}</ul>
        </div>
