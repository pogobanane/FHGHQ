          ...
          for (var j = 0; j < region.data.mapItems.length; j++) {
            ...
            if (
              ...
              let distance = Math.sqrt(
                Math.pow(this.props.obj.y - item.y, 2) +
                  Math.pow(this.props.obj.x - item.x, 2)
              );
    ...
    if (obj.flags & 0x02) {
      addon_hq = (
        <L.Marker
          position={position}
          icon={
            markers.GenerateHQIcon(obj.teamId) /*markers.iconarray[4][side()]*/
          }
        />
      );
    }
      ...
      case 'town':
        ...
        for (var i = 0; i < this.props.events.length; i++) {
          ...
          if (this.props.regionId == event.region) {
            ...
            if (
              this.props.obj.x == item.x &&
              this.props.obj.y == item.y &&
              !foundevent
            ) {
        ...
        tooltip = (
          <L.Tooltip className='tooltip' opacity={1}>
            <p>
              <b>{this.props.name}</b>
            </p>
            {eventstring}
            <div className='row tooltip_row'></div>
            <br />
