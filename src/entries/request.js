...
$(() => {
  ...
  class Modal extends React.Component {
    ...
    SubmitData() {
      $.post(`/requestPassword?${$.param({ globalid, password: this.state.password })}`, (response) => {
        if (response === 'wrong') {
        ...
        if (this.props.info.secure == 0) {
          body = (
            <div>
              <h5>
                Password:
                <input type="text" name="password" onChange={(e) => this.handleChangeField(e)} />
              </h5>
