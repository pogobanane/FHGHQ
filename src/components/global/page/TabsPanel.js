...
class OpTimer_ extends React.Component {
  ...
  submitTimer() {
    ...
    if (!isNaN(date.getTime())) {
      ...
      socket.emit('submitOpTimer', { date });
    }
  }
  ...
  render() {
    ...
    function GetDateString(date) {
      ...
      const string = `${day} ${month} - ${hour}:${minute} `;
      return string;
    }

    let totalstring = 'OP starts in ----';
    if (this.props.settings.optimer != undefined) {
      const props = new Date(this.props.settings.optimer);
      date = GetDateString(props);
      let time = '';
      const timerdate = new Date(props).getTime();
...
class Top_ extends React.Component {
  ...
  render() {
    ...
    for (let i = 0; i < this.props.users.users.length; i++) {
      const user = this.props.users.users[i];
