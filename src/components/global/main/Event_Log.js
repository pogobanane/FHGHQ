...
class Events extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    let soundsArray = [
      /*{id:0,faction:"COLONIALS"},{id:1,faction:"WARDENS"}*/
    ];
    for (var i = 0; i < this.props.events.length; i++) {
      ...
      if (
        (prevItem.teamId != newItem.teamId && !newItem.flags & 4) ||
        (prevItem.flags & 4 && !(newItem.flags & 4) && newItem.iconType == 5)
      ) {
        if (newItem.teamId != 'NONE') {
          let townname = U.GetTownName(
            newEvent.region,
            prevItem,
            this.props.static
          );
  ...
  GetEventString(obj) {
    ...
    if (prevItem.teamId != newItem.teamId) {
      if (newItem.teamId == 'NONE') {
        ...
        if (newItem.flags & 0x10) {
          actionstring = (
            <span className='eventlog_text_nuke'>was nuked by </span>
          );
      ...
      if (newItem.iconType == 5) {
        ...
        actionstring = (
          <span className='eventlog_text_upgrade'>{actionstring}</span>
        );
        faction = GetFaction(newItem, false);
      }
      ...
      actionstring = (
        <span className='eventlog_text_nuke'>was nuked by Someone</span>
      );
    } else if (
      prevItem.iconType == newItem.iconType &&
      !(prevItem.flags & 4) &&
    ...
    let totalstring = (
      <span>
        ...
        {regionName}:{' '}
        <b
          className='eventlog_town_name'
          onClick={() => this.handleSelect(prevItem, townname, obj.region)}
        >
          {townname}
        </b>{' '}
        {actionstring}
    ...
    return (
      <li className='event_log_line' key={'event' + obj.date}>
        {totalstring}
      </li>
    );
  }
  ...
  GetEventString(obj) {
    ...
    switch (obj.type) {
      case 0:
        actionstring = (
          <span>
            <img style={{ width: 15, height: 15 }} src={tech[obj.packet].url} />{' '}
            {tech[obj.packet].name} research complete
          </span>
        );
        break;
      case 1:
        ...
        let username = (
          <b>{U.GetUsername(this.props.users.users, packet.wip.author)}</b>
        );
        let wiparray = [];
        for (var i = 0; i < packet.wip.request.length; i++) {
          ...
          wiparray.push(
            <span
              key={'wiparray' + obj.date + '|' + item.catid + '|' + item.itemid}
            >
              {item.crates}x{' '}
              <img
                style={{ width: 15, height: 15 }}
                src={cost.cost[item.catid][item.itemid].src}
              />{' '}
            </span>
          );
        }
        let location = (
          <img
            style={{ width: 15, height: 15 }}
            src='https://cdn.glitch.com/6393f3fd-16a7-4641-ae3d-994f8e7cea4e%2FIconStatusEncumberedRed.png?1549573551066'
            onClick={() =>
              store.dispatch(A.selectObject('requests', packet.request))
            }
          />
        );
        ...
        actionstring = (
          <span>
            {username} delivered {wiparray} to {location}
          </span>
        );
        break;
      case 2:
        actionstring = (
          <span>
            <img
              style={{ width: 15, height: 15 }}
              src='https://cdn.glitch.com/dd3f06b2-b7d4-4ccc-8675-05897efc4bb5%2FIconStatusEncumbered.png?1554591180726'
              onClick={() =>
                store.dispatch(A.selectObject('requests', obj.packet))
              }
            />{' '}
            completed
          </span>
      ...
      case 3:
        actionstring = (
          <span>
            New supply order requested{' '}
            <img
              id='eventlog_logi_img'
              src='https://cdn.glitch.com/6393f3fd-16a7-4641-ae3d-994f8e7cea4e%2FIconStatusEncumberedRed.png?1549573551066'
              onClick={() =>
                store.dispatch(A.selectObject('requests', obj.packet))
              }
    ...
    return (
      <li className='event_log_line' key={'privateevent' + obj.date}>
        {totalstring}
      </li>
    );
  }
