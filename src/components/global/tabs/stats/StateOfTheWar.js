...
class StateOfTheWar extends React.Component {
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (
      JSON.stringify(nextProps.state) != JSON.stringify(this.props.state) ||
      JSON.stringify(nextProps.counters) !=
        JSON.stringify(this.props.counters) ||
  ...
  render() {
    ...
    if (this.props.timerOn) {
      try {
        ...
        counters = [
          timelinesnap.total[1],
          timelinesnap.total[2],
          timeline.total[0][0] + timeline.total[0][1] + timeline.total[0][2],
        ];
    ...
    percentages[0] = (counters[0] / counters[2]) * 100;
    percentages[1] = (counters[1] / counters[2]) * 100;
    percentages[2] = { width: 100 - percentages[0] - percentages[1] + '%' };
    percentages[0] = { width: percentages[0] + '%' };
    percentages[1] = { width: percentages[1] + '%' };
    return (
      <React.Fragment>
        <div className='progress' id='stats_progress'>
          ...
          <div className='progress-bar white-bg' style={percentages[2]}></div>
        ...
        <div className='stats_table_towns'>
          <table>
            <tbody>
              <tr>
                {GetCondition(objimg[0].n, state[1][0], state[2][0])}{' '}
                {GetCondition(objimg[1].n, state[1][1], state[2][1])}
                {GetCondition(objimg[2].n, state[1][2], state[2][2])}{' '}
                {GetCondition(objimg[3].n, state[1][3], state[2][3])}
              </tr>
              <tr>
                {GetCondition(objimg[4].n, state[1][4], state[2][4])}{' '}
                {GetCondition(objimg[5].n, state[1][5], state[2][5])}
                {GetCondition(objimg[6].n, state[1][6], state[2][6])}{' '}
                {GetCondition(objimg[7].n, state[1][7], state[2][7])}
              </tr>
...
function GetCondition(url, number, number2) {
  ...
  return (
    <td>
      <img className='stats_stateofthewar_img' src={url} />
      <p className={textclass}>
        <span className='collie_text'>
          <AnimatedNumber value={number} duration={600} stepPrecision={0} />
        </span>
        <span id='separator'>-</span>
        <span className='warden_text'>
          <AnimatedNumber value={number2} duration={600} stepPrecision={0} />
        </span>
      </p>
    </td>
  );
