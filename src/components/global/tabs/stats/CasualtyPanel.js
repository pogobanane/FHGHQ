...
class CasualtyPanel extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    let oldindex =
      Math.ceil((this.props.time - this.props.timerStart) / 600000) + 1;
    let newindex =
      Math.ceil((nextProps.time - nextProps.timerStart) / 600000) + 1;
  ...
  render() {
    ...
    try {
      ...
      for (let i in cas.json.wr) {
        ...
        cas.list.push({
          name: RegionImages.regionlist[i].name,
          c: region.c,
          w: region.w,
          index: i,
        });
      ...
      try {
        cas.hourlist.sort(compareRegions);
        cas.hourlist = cas.hourlist.slice(0, 6);
        for (let i = 0; i < cas.hourlist.length; i++) {
          let el = cas.hourlist[i];
          cas.hourlist[i] = (
            <CasualtyTR
              key={'stats_castable_hour_' + el.name}
              el={el}
              cellclass='stats_castable_hour_td'
            />
          );
        }
        while (cas.hourlist.length < 6) {
          cas.hourlist.push(
            <tr key={'stats_castable_hour_' + cas.hourlist.length}>
              <td></td>
            </tr>
          );
        }
      ...
      for (let i = 0; i < cas.list.length; i++) {
        ...
        cas.list[i] = (
          <CasualtyTR
            key={'stats_castable_top_' + el.name}
            el={el}
            cellclass='stats_castable_top_td'
            active={displayedregions.includes(Number(el.index))}
          />
        );
      }
      cas.wh = cas.w - cas.wh;
    ...
    if (cas.c >= cas.w) {
      totalarrays.cumulative.push(
        <li className='collie_text'>{GetAnimNumber(cas.c)}</li>
      );
      totalarrays.cumulative.push(
        <li className='warden_text'>{GetAnimNumber(cas.w)}</li>
      );
    } else {
      totalarrays.cumulative.push(
        <li className='warden_text'>{GetAnimNumber(cas.w)}</li>
    ...
    if (cas.ch >= cas.wh) {
      totalarrays.hour.push(
        <li className='collie_text'>{GetAnimNumber(cas.ch)}/h</li>
      );
      totalarrays.hour.push(
        <li className='warden_text'>{GetAnimNumber(cas.wh)}/h</li>
      );
    } else {
      totalarrays.hour.push(
        <li className='warden_text'>{GetAnimNumber(cas.wh)}/h</li>
    ...
    return (
      <div id='stats_table_cas' className='stats_div'>
        <p className='stats_div_header'>Casualties</p>
        <table id='stats_table_cas_table' className='stats_numbers_table'>
          <tbody>
            <tr id='stats_casualtytop_tr_colonials'>
              <td className='collie_text'>{/*Colonials*/}</td>
              <td className='stats_table_totalcas_total'>
                <FlipMove.default
                  ...
                  duration={0}
                >
                  {totalarrays.cumulative}
                </FlipMove.default>
              </td>
              <td className='stats_table_totalcas_hour'>
                <FlipMove.default
                  typeName='ul'
                  enterAnimation='accordionVertical'
                  staggerDelayBy='0'
            ...
            <tr id='stats_casualtytop_tr_total'>
              <td>Total</td>
              <CasualtyTD
                tdclass='stats_table_totalcas_total'
                number={cas.total}
              />
              <CasualtyTD
                tdclass='stats_table_totalcas_hour'
                number={Number(cas.wh) + Number(cas.ch)}
                addon='/h'
              />
            </tr>
          </tbody>
  ...
  render() {
    return (
      <td className={this.props.tdclass}>
        {GetAnimNumber(this.props.number)}
        {this.props.addon}
      </td>
    );
  }
...
class CasualtyTR extends React.Component {
  ...
  render() {
    ...
    if (el.c + el.w >= 20) {
      return (
        <tr key={'stats_castable_top_' + el.name}>
          <td className={nameclass}>{el.name}</td>
          <td className={'collie_text ' + this.props.cellclass}>
            {GetAnimNumber(el.c)}
          </td>
          <td className={'warden_text ' + this.props.cellclass}>
            {GetAnimNumber(el.w)}
          </td>
        </tr>
      );
...
function compareRegions(a, b) {
  if (Number(a.c) + Number(a.w) < Number(b.c) + Number(b.w)) return 1;
  if (Number(a.c) + Number(a.w) > Number(b.c) + Number(b.w)) return -1;
  return 0;
}
...
function GetAnimNumber(value) {
  return (
    <AnimatedNumber
      ...
      formatValue={(value) => U.FormatNumber(value)}
    />
  );
}
