...
class MapIcons_ extends React.Component {
  ...
  GetRange(obj, regionId, range = 150) {
    ...
    return (
      <L.Circle
        ...
        opacity={1}
        fillOpacity={0.2}
        radius={RegionImages.ratio * range}
      />
    );
  }
  ...
  GetPrivateIcons() {
    ...
    if (display.custom.requests) {
      for (var request in roominfo.requests) {
        ...
        objects.push(
          <MO.RequestIcon key={'request' + request} request={obj} />
        );
      }
    }
  ...
  GetDynamicIcons() {
    ...
    return this.props.zoom >= 4.5 && display.ranges
      ? iconlist.concat(ranges)
      : iconlist;
  }
