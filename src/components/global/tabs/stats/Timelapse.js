...
//////////////////////////////////////////////
class Timelapse_ extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (
      JSON.stringify(this.props.dynamic) == JSON.stringify(nextProps.dynamic)
    ) {
  ...
  render() {
      ...
      for (let j = 0; j < region.data.mapItems.length; j++) {
        ...
        switch (item.iconType) {
          ...
          case 29:
            ...
            markerlist.push(
              <Icon
                key={'stats_map_marker_' + item.x + item.y}
          ...
          case 32:
            resourcelist.push(
              <ResourceIcon
                key={'stats_map_marker_resource+' + item.x + item.y}
                regionId={region.regionId}
                item={item}
              />
            );
    ...
    let totaldatestring = (
      <p id='stats_timelapse_date'>
        {datestring[1]} {datestring[2]} {datestring[3]} {timestring[0]}:
        {timestring[1]}
      </p>
    );
    ...
    for (let i = 0; i < this.props.stats.currentwar.warstats.length; i++) {
      ...
      wars.push(
        <option key={'stats_warselect_' + war.warNumber} value={war.warNumber}>
          War {war.warNumber}
        </option>
      );
    }
    wars.reverse();
    return (
      <div id='stats_map_container_inner' className='col'>
        <L.Map
          ref='worldmap'
        ...
        >
          ...
          <LeafletControl.default
            position='topright'
            className='leaflet_stats_datecontrol'
          >
            {totaldatestring}
            <button
              className='stats_timelapse_btn'
              onClick={() => this.props.PlayTimelapse()}
            >
              {this.props.timerOn ? (
                <img
                  className='stats_timelapse_btn_img'
                  src='https://cdn.glitch.com/84b19724-a86b-4caa-8e69-1e9c973e043f%2Fpause.png?v=1566737620901'
                />
              ) : (
                <img
                  className='stats_timelapse_btn_img'
                  src='https://cdn.glitch.com/84b19724-a86b-4caa-8e69-1e9c973e043f%2Fplay.png?v=1564333525644'
                />
              )}
            </button>
            <button
              className='stats_timelapse_btn'
              onClick={() => this.props.StopTimelapse()}
            >
              <img
                className='stats_timelapse_btn_img'
                src='https://cdn.glitch.com/84b19724-a86b-4caa-8e69-1e9c973e043f%2Fstop.png?v=1564333527433'
              ></img>
            </button>
            <select
              id='stats_timelapse_warselect'
...
class Tiles extends React.Component {
  ...
  render() {
    ...
    for (let i = 0; i < RegionImages.regionlist.length; i++) {
      ...
      let fillopacity = 0;
      let o = region.center;
      let k = RegionImages.k;
      let w = RegionImages.w;
      if (region.name != '') {
        hexarray.push(
          <L.Polygon
            positions={[
  ...
  render() {
    ...
    if (item.flags & 0x02) {
      addon_hq = (
        <L.Marker
          position={position}
          icon={
            markers.GenerateHQIcon(
              item.teamId
            ) /*markers.iconarray[4][this.GetTeamId()]*/
          }
          opacity={0.6}
        />
      );
    ...
    return (
      <React.Fragment>
        ...
        {addon_hq}
        <L.Marker
          key={'stats_timelapse_icon_' + item.x + item.y}
          position={position}
          icon={icon}
          zIndexOffset={400}
        />
  ...
  render() {
    ...
    return (
      <L.CircleMarker
        ...
        fillOpacity={1}
        radius={3}
        className={'stats_timelapse_resource_marker'}
        weight={1}
      />
    );
  }
...
class FortIcon extends React.Component {
  ...
  render() {
    ...
    return (
      <L.CircleMarker
        key={'stats_timelapse_forticon' + item.teamId + item.x + item.y}
        center={position}
        fillColor={color}
        color={'#000000'}
        radius={4}
...
//////
const mapStateToPropsTimelapse = (store) => {
