...
class RequestContainer extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (
      ...
      nextProps.selected.type != 'requests'
    ) {
      return false;
    }
    if (
      JSON.stringify(this.props.requests[this.props.selected.key]) !=
  ...
  SubmitItems() {
    ...
    if (CheckCompleted()) {
      let packet = {
        type: 2,
        date: JSON.stringify(new Date()),
        packet: this.props.selected.key,
      };
      store.dispatch(A.submitEvent(packet));
      socket.emit('submitEvent', packet);
    }
  ...
  render() {
    ...
    if (
      ...
      this.props.selected.key == undefined ||
      this.props.requests[this.props.selected.key] == undefined
    ) {
      return null;
    }
    ...
    for (
      var i = 0;
      i < this.props.requests[this.props.selected.key].wip.length;
      i++
    ) {
      ...
      if (window.steamid != wip.author && wip.request.length > 0) {
        ...
        if (otherProductionCounter % 2 == 1) {
          margin = 'other_production_left';
        }
    ...
    let myproduction = FindMyProduction(
      this.props.requests[this.props.selected.key].wip
    );
    if (myproduction != -1) {
      if (
        this.props.requests[this.props.selected.key].wip[myproduction].request
          .length > 0
      ) {
        ...
        {this.props.selected.type == 'requests' ? (
          <React.Fragment>
            <div id='requestcardcontainer'>
              <RequestCard
                ref={(card) => {
                  window.requestcard = card;
                }}
                users={this.props.users}
                request={this.props.requests[this.props.selected.key]}
                signature={this.props.selected.key} /*What needs to be done?*/
              />
            </div>
...
class RequestCard extends React.Component {
  ...
  GetPriority(index, totalwip, done) {
    ...
    function GetLine(array, y) {
      ...
      let itemwip = totalwip.find(
        (item) => obj.catid == item.catid && obj.itemid == item.itemid
      );
      let doneWidth = 0;
      let wipWidth = 0;
      let abbrDoneCrates = 0;
      let abbrWipCrates = 0;
      let abbrDoneAmount = 0;
      let abbrWipAmount = 0;
      ...
      if (itemwip != undefined) {
        ...
        abbrWipAmount =
          itemwip.crates * cost.cost[itemwip.catid][itemwip.itemid].i;
        if (doneWidth < 100) {
          //SCANNING IN 'WIP' ARRAY
      ...
      return (
        ...
        >
          <td className='progress pb_request_container'>
            <div
              className={
                'progress-bar progress-bar-striped pb_request pb_request_done ' +
                alignclass
              }
            ...
            <div
              className={
                'progress-bar progress-bar-striped progress-bar-animated pb_request pb_request_wip ' +
                alignclass
              }
              style={{ width: wipWidth + '%' }}
            ></div>
          </td>
          <td className='reqtd' style={{ width: 20 }}>
            <img
              className='card_req_itemimage'
              src={cost.cost[obj.catid][obj.itemid].src}
            />
          </td>
          <td className='reqtd'>{cost.cost[obj.catid][obj.itemid].name}</td>
          <td className='reqtd' style={{ width: 30 }}>
            <abbr
              title={
                'Required: ' +
                obj.crates +
          ...
          <td className='reqtd' style={{ width: 40 }}>
            <abbr
              title={
                'Required: ' +
                obj.amount +
    ...
    return (
      <React.Fragment>
        <div
          ...
          href={'#cardreq' + headertype}
          className={'card-header cardheader ' + headerclass}
        >
          <table>
            <thead>
              <tr>
                <th></th>
                <th style={{ width: 20 }}>
                  <i className='fas fa-chevron-down align-right' />
                </th>
                <th>{headertype + ' Priority'}</th>
                <th style={{ width: 30 }}>
                  <img
                    className='card_req_manufacturerimage'
        ...
        <div
          className='card-body cardheader collapse show'
          id={'cardreq' + headertype}
        >
            ...
            <table>
              <tbody>
                <tr>
                  <td style={{ width: 27 }}>
                    <button
                      className='card_req_manufacturerbtn'
                      onClick={() =>
                        window.requestmodalcontainer.ShowModal(
                    ...
                    >
                      <img
                        className='card_req_manufacturerimage'
                        src='https://cdn.glitch.com/6393f3fd-16a7-4641-ae3d-994f8e7cea4e%2FIconFilterUtility.png?1548935226605'
                      />
                    </button>
                  </td>
                  <td
                    className='card_ambush_0cell noclip'
                    style={{ minWidth: 50 }}
                  >
                    {'Request by ' +
                      U.GetUsername(
                        this.props.users.users,
                        this.props.request.author
                      )}
                  </td>
                  <td style={{ width: 24 }}>
                    <Popover.default
...
class MyProductionCard extends React.Component {
  ...
  GetLine(index) {
    ...
    return (
      <tr style={{ height: 26 }}>
        <td
          className='card_ambush_removebutton useronly'
          style={{ width: 24 }}
          onClick={() => this.props.RemoveItem(index)}
        >
          <img
            className='card_ambush_removeimage'
            src='https://cdn.glitch.com/dd3f06b2-b7d4-4ccc-8675-05897efc4bb5%2Ffdsgafd.png?1556797745838'
          />
        </td>
        <td>
          <img
        ...
        <td style={{ width: 60 }}>
          <input
            ...
            onChange={(event) => this.props.handleAmountChange(event, index)}
            className='towninput'
          ></input>
        </td>
      </tr>
      ...
      return (
        <div className='card'>
          ...
          >
            <button
              type='button'
              className='btn card_req_submitmenubtn'
              onClick={() =>
                this.setState({ isPopoverOpen: !this.state.isPopoverOpen })
              }
            >
              <img
                className='card_req_submitmenu_img'
                src='https://cdn.glitch.com/dd3f06b2-b7d4-4ccc-8675-05897efc4bb5%2Fgsds.jpg?1555235385083'
              />
            </button>
          </Popover.default>
          <div className='card-header cardheader'>Your production</div>
          <div className='card-body cardheader'>
            <table className='table'>
              ...
              <tbody id='yourprodtbody'>{prodlines}</tbody>
            </table>
          </div>
        </div>
      );
...
class OtherProductionCard extends React.Component {
  ...
  render() {
    ...
    return (
      <div className={'card other_production_card ' + this.props.margin}>
        <div className='card-header cardheader other_production_cardheader noclip'>
          ...
          <span className='other_production_headertext'>
            {this.props.author}
          </span>
        </div>
        <div className='card-body cardheader'>
