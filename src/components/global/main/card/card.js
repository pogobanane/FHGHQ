...
class Card_ extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (
      !checktype(this.props.selected.type) &&
      !checktype(nextProps.selected.type)
    ) {
      return false;
    }
  ...
  render() {
    ...
    if (
      this.props.selected.type != 'refinery' &&
      this.props.selected.type != 'production' &&
      this.props.selected.type != 'storage' &&
      this.props.selected.type != 'stockpiles' &&
      this.props.selected.type != 'icon' &&
      this.props.selected.type != 'fobs'
    ) {
      return null;
    }
    ...
    switch (this.props.selected.type) {
      case 'stockpiles':
        ...
        if (this.props.settings.link != undefined) {
          title = (
            <div id='card_notify_headercontainer'>
              <span id='card_header_title'>{title}</span>{' '}
              <div id='card_notify_container'>
                Notify in Discord
                <input
                  type='checkbox'
      ...
      case 'fobs':
        title = (
          <React.Fragment>
            <span className='fob_card_text'>Outpost</span>
          </React.Fragment>
        );
        body = (
          <React.Fragment>
      ...
      case 'icon':
        title = (
          <React.Fragment>
            {markers.MiscIconArray[this.props.selected.misctype].name}{' '}
            <UR.DeletePopover
              header={'Delete icon?'}
              type={'misc_icon'}
              signature={this.props.selected.key}
            />
          </React.Fragment>
        );
        body = null;
...
export const Card = connect(mapStateToProps, null, null, { forwardRef: true })(
  Card_
);
