...
class MiscIcon_ extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (
      (this.props.selected.key == this.props.signature &&
        nextProps.selected.key != nextProps.signature) ||
      (this.props.selected.key != this.props.signature &&
        nextProps.selected.key == nextProps.signature)
    ) {
      return true;
    ...
    if (
      JSON.stringify(this.props.private[this.props.signature]) !=
      JSON.stringify(nextProps.private[nextProps.signature])
    ) {
      return true;
    }
  ...
  handleDragEnd(e) {
    ...
    let packet = {
      position: { x: coords.lng, y: coords.lat },
      notes: obj.notes,
      type: obj.type,
      lastupdate: new Date(),
    };
  ...
  GetArtyIcon(obj) {
    ...
    let icon = new markers.MiscIcon({
      iconUrl: markers.MiscIconArray[obj.type].url,
      shadowUrl: markers.MiscIconArray[obj.type].shadow,
    ...
    if (this.state.artyaddon != null) {
      ...
      let distanceraw = [
        this.state.artyaddon[0] - obj.position.y,
        this.state.artyaddon[1] - obj.position.x,
      ];
      ...
      let distance = Math.floor(
        Math.sqrt(Math.pow(distanceraw[0], 2) + Math.pow(distanceraw[1], 2)) /
          ratio
      );
      if (distance < artysize / 2) {
        let text = distance + ' m<br />' + azimuth + '°';
        var IconText = NativeL.divIcon({
          className: 'note_icon_arty',
          html:
            "<div className='note_arty_text_container'><span class='note_arty_text'><b>" +
            text +
            '</b></span></div>',
        });
          ...
          <React.Fragment>
            <L.Polyline
              ...
              positions={[
                ...
                this.state.artyaddon,
              ]}
            />
            <L.Marker
              position={[this.state.artyaddon[0], this.state.artyaddon[1]]}
              icon={IconText}
              opacity={1}
              zIndexOffset={1000}
            />
  ...
  render() {
    ...
    if (text != '') {
      text = JSON.parse(
        JSON.stringify(this.props.private[this.props.signature].notes)
      );
      text = text.replace(/(?:\r\n|\r|\n)/g, '<br>');
    }
    var IconText = NativeL.divIcon({
      ...
      html:
        "<div className='note_icon_text_container'><span class='note_icon_text'><b>" +
        text +
        '</b></span></div>',
    });
    IconText.options.iconSize = [undefined, undefined];
