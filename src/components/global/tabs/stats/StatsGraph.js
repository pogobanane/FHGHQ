...
class StatsGraph extends React.Component {
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (
      //JSON.stringify(this.props.timeline)!=JSON.stringify(nextProps.timeline)||
  ...
  render() {
    ...
    let selectedlength =
      Math.ceil((this.props.time - this.props.timerStart) / this.props.scale) +
      1;
        ...
        let casstring =
          payload[0].value > 0 ? (
            <span className='collie_text'>
              {payload[0].value} more colonial casualties
            </span>
          ) : (
            <span className='warden_text'>
              {Math.abs(payload[0].value)} more warden casualties
            </span>
          );
        return (
    ...
    return (
      <div style={{ width: '100%', height: '50%' }}>
        ...
        <Recharts.ResponsiveContainer>
          ...
          >
            <Recharts.XAxis dataKey='x' hide={true} />
            <Recharts.YAxis hide={true}></Recharts.YAxis>
            <Recharts.Tooltip content={<CasTooltip />} />
            <defs>
              <linearGradient id='splitColor' x1='0' y1='0' x2='0' y2='1'>
                <stop
                  offset={gradientOffset(casgraph)}
                  stopColor='#65875e'
                  stopOpacity={1}
                />
                <stop
