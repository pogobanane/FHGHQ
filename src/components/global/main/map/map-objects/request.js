...
class RequestIcon_ extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (
      JSON.stringify(this.props.requests[signature]) !=
      JSON.stringify(nextProps.requests[signature])
    ) {
      return true;
    }
    return false;
  ...
  handleDragEnd(e) {
    ...
    store.dispatch(
      A.deleteObject('requests', U.signature(this.props.request.position))
    );
    socket.emit('updateObject', {
      type: 'requests',
      object: packet,
      key: U.signature(position),
    });
    socket.emit('deleteObject', {
      type: 'requests',
      key: U.signature(this.props.request.position),
    });
  }
