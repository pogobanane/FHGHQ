...
class StatsCore extends React.Component {
      ...
      if (request.status >= 200 && request.status < 400) {
        try {
          ...
          if (data.startpoint.number == comp.state.warnumber) {
            data.reports = FillReportGaps(
              data.reports,
              data.startpoint.date,
              Date.now(),
              data.startpoint
            );
            comp.setState({
          ...
          document.getElementById('stats_timelapse_loader').style.display =
            'none';
        } catch (err) {}
      }
    };
    ...
    try {
      ...
      for (let i = 0; i < mapstate[event.rId].length; i++) {
        ...
        if (event.x == item.x && event.y == item.y) {
          ...
          if (
            (event.niT == 5 || event.niT == 6 || event.niT == 7) &&
            event.f & 0x01
          ) {
            if (team != item.teamId) {
              if (team == 'NONE') {
                timeline.wincondition[timeindex][getTeamId(item) - 1][
                  item.iconType - 5
                ]--;
                ...
                timeline.wincondition[timeindex][event.ntId - 1][
                  event.niT - 5
                ]++;
              }
          ...
          if (event.niT == 5 || event.niT == 6 || event.niT == 7) {
            if (team != item.teamId && team != 'NONE') {
              if (
                timeline.spec.toptowns[timeindex][
                  event.rId + event.x + event.y
                ] == undefined
              ) {
                timeline.spec.toptowns[timeindex][
                  event.rId + event.x + event.y
              ...
              timeline.spec.toptowns[timeindex][event.rId + event.x + event.y]
                .amount++;
              timeline.spec.toptowns[timeindex][
                event.rId + event.x + event.y
              ].side = event.ntId;
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (
      props.stats.warstats != undefined &&
      props.tab.tab == 5 &&
      ...
      if (currentwar.startpoint != undefined) {
        if (currentwar.startpoint.date != null) {
          currentwar.reports = FillReportGaps(
            currentwar.reports,
            currentwar.startpoint.date,
            Date.now(),
            currentwar.startpoint
          );
        }
        return {
          warnumber: currentwar.startpoint.number,
  ...
  render() {
    ...
    return (
      <StatsGraphic
        stats={
          this.props.stats
        } /*dynamic={dynamic} SwitchWar={this.SwitchWar} condition={condition} timeline={timeline} counters={counters} state={state} startpoint={startpoint} scount={scount} scale={scale} previous={this.state.previous} 
...
class StatsGraphic extends React.Component {
  ...
  PlayTimelapse() {
    if (this.state.timerOn) {
      ...
      this.timer = setInterval(() => {
        if (
          this.state.time >
          Date.now() /*||this.props.previous&&this.state.time>this.props.EndTime*/
        ) {
          ...
          if (
            (this.props.previous &&
              newtime > this.props.currentwar.warstats.conquestEndTime) ||
            newtime > Date.now()
          ) {
            this.StopTimelapse();
          }
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <p>
          Statistics {/*<span id="statsp">Press F11 for Fullscreen</span>*/}
        </p>
        {/*
       <div  className="row">  
      ...
      <div className="col">
        ...
        <WarHistory
          warhistory={this.props.stats.currentwar.warstats}
          oldwars={oldwars}
        />
        {/*</div>
      </div>
...
class PlayersGraph extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (
      JSON.stringify(this.props.stats.currentwar.reports.length) !=
        JSON.stringify(nextProps.stats.currentwar.reports.length) ||
      this.props.timerOn != nextProps.timerOn ||
      (nextProps.timerOn &&
        JSON.stringify(nextProps.time) != JSON.stringify(this.props.time))
    ) {
      return true;
    }
    return false;
    ...
    if (this.props.previous && this.props.time == this.props.timerStart) {
      for (let i = 0; i < this.props.stats.currentwar.reports.length; i++) {
        try {
          datapoints.push({
            x: i,
            y: this.props.stats.currentwar.reports[i].pop,
          });
        } catch (err) {}
      }
    } else {
      if (this.props.timerOn) {
        timeindex = Math.ceil(
          (this.props.time - this.props.timerStart) / 600000
        );
        try {
      ...
      let startindex =
        timeindex > this.state.dayscale * this.state.oneday
          ? timeindex - this.state.dayscale * this.state.oneday
          : 0;
...
class StructureCount extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    try {
      if (
        JSON.stringify(nextProps.scount) != JSON.stringify(this.props.scount) ||
        this.props.timerOn != nextProps.timerOn ||
        (nextProps.timerOn &&
          JSON.stringify(nextProps.timelinesnap) !=
            JSON.stringify(this.props.timelinesnap))
      ) {
  ...
  render() {
    ...
    for (let i = 0; i < objimg.length; i++) {
      ...
      function GetCell(src, amount) {
        return (
          <td>
            <img className='stats_scount_img' src={src} />
            <span className='stats_scount_span'>
              <AnimatedNumber value={amount} duration={600} stepPrecision={0} />
            </span>
          </td>
        );
      }
      structurecounttr.push(
        <tr className='stats_scount_tr'>
          {GetCell(item.c, scount.each[item.icontype].c)}
          {GetCell(item.w, scount.each[item.icontype].w)}
        </tr>
      );
    }
    return (
      <div id='stats_table_scount' className='col stats_div'>
        <p className='stats_div_header'>Captured Structures</p>
        <table>
          <tbody>
            <tr>
              <td className='collie_text'>
                <AnimatedNumber
                  value={scount.captured.c}
                  duration={600}
                  stepPrecision={0}
                />
              </td>
              <td className='warden_text'>
                <AnimatedNumber
                  value={scount.captured.w}
                  duration={600}
                  stepPrecision={0}
                />
              </td>
            </tr>
            <tr id='stats_scount_separator'>
              <td></td>
              <td></td>
            </tr>
            {structurecounttr}
...
class WarStats extends React.Component {
  //Conquest X, Day Y Underway since
  constructor(props) {
    super(props);
    this.state = {
      ///State
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (
      this.props.stats.warstats.warNumber !=
        nextProps.stats.warstats.warNumber ||
      this.props.stats.warstats.conquestEndTime !=
        nextProps.stats.warstats.conquestEndTime
    ) {
      return true;
    }
    /*
  ...
  SplitTime(time) {
    ...
    var timestring =
      diffDays + ' : ' + diffHrs + ' : ' + diffMins + ' : ' + diffSec;
    return timestring;
  }
  render() {
    let day = 0;
    ...
    try {
      ...
      if (this.props.stats.warstats != undefined) {
        ...
        if (
          this.props.stats.warstats.conquestEndTime > 1 &&
          this.props.stats.warstats.conquestEndTime >
            this.props.stats.warstats.conquestStartTime
        ) {
          ...
          let conquesttime = this.SplitTime(
            this.props.stats.warstats.conquestEndTime -
              this.props.stats.warstats.conquestStartTime
          );
          victoryaddon = (
            <p>
              {victoryfaction} have won Conquest {warnumber} in {conquesttime}
            </p>
          );
          if (
            this.props.stats.warstats.resistanceStartTime >
            this.props.stats.warstats.conquestEndTime
          ) {
            var time =
              new Date().getTime() -
              new Date(this.props.stats.warstats.resistanceStartTime).getTime();
          ...
          var time =
            new Date().getTime() -
            new Date(this.props.stats.warstats.conquestStartTime).getTime();
          wartime = this.SplitTime(time);
        }
      }
    } catch (err) {}
        ...
        <div className='stats_table_towns'>
          <table>
            <tbody>
              <tr>
                {GetCondition(
                  'https://raw.githubusercontent.com/the-fellowship-of-the-warapi/Assets/master/Map%20Icons/Bases/MapIconStaticBase1Colonial.png',
                  condition[0][0]
                )}
                {GetCondition(
                  'https://raw.githubusercontent.com/the-fellowship-of-the-warapi/Assets/master/Map%20Icons/Bases/MapIconStaticBase2Colonial.png',
                  condition[0][1]
                )}
                {GetCondition(
                  'https://raw.githubusercontent.com/the-fellowship-of-the-warapi/Assets/master/Map%20Icons/Bases/MapIconStaticBase3Colonial.png',
                  condition[0][2],
                  <span>
                    (
                    <span className='stats_table_counter_special'>
                ...
                {GetCondition(
                  'https://cdn.glitch.com/dd3f06b2-b7d4-4ccc-8675-05897efc4bb5%2FCivicCenterColonial.png?v=1560693262971',
                  condition[0][3],
                  <React.Fragment>
                    /
                    <span className='stats_table_counter_special'>
              ...
              <tr>
                {GetCondition(
                  'https://raw.githubusercontent.com/the-fellowship-of-the-warapi/Assets/master/Map%20Icons/Bases/MapIconStaticBase1Warden.png',
                  condition[1][0]
                )}
                {GetCondition(
                  'https://raw.githubusercontent.com/the-fellowship-of-the-warapi/Assets/master/Map%20Icons/Bases/MapIconStaticBase2Warden.png',
                  condition[1][1]
                )}
                {GetCondition(
                  'https://raw.githubusercontent.com/the-fellowship-of-the-warapi/Assets/master/Map%20Icons/Bases/MapIconStaticBase3Warden.png',
                  condition[1][2],
                  <span>
                    (
                    <span className='stats_table_counter_special'>
                ...
                {GetCondition(
                  'https://cdn.glitch.com/dd3f06b2-b7d4-4ccc-8675-05897efc4bb5%2FCivicCenterWarden.png?v=1560693266377',
                  condition[1][3],
                  <React.Fragment>
                    /
                    <span className='stats_table_counter_special'>
...
class WinCount extends React.Component {
  ...
  render() {
    ...
    return (
      <div className='stats_div' id='stats_wincount'>
        <p className='stats_div_header'>Wins</p>
        <table id='stats_win_table'>
          <tbody>
            <tr>
              <td>
                <img
                  className='stats_faction_img'
                  src='https://cdn.glitch.com/84b19724-a86b-4caa-8e69-1e9c973e043f%2Fcoll.png?v=1567263083742'
                />
              </td>
              <td>
                <img
                  className='stats_faction_img'
                  src='https://cdn.glitch.com/84b19724-a86b-4caa-8e69-1e9c973e043f%2Fward.png?v=1567263084512'
                />
              </td>
            </tr>
            <tr>
...
function GetCondition(url, number, addon, total) {
  ...
  return (
    <td>
      <img className='stats_condition_img' src={url} />
      <p className={textclass}>
        <AnimatedNumber value={number} duration={600} stepPrecision={0} />
        {addon}
      </p>
    </td>
  );
