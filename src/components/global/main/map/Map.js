  ...
  render() {
    ...
    for (let i = 0; i < RegionImages.regionCounter; i++) {
      ...
      for (let j = 0; j < this.props.roominfo.static[i].data.mapTextItems.length; j++) {
        ...
        const icon = <L.Marker position={position} icon={myIcon} />;
        labels.push(icon);
      }
    }
    ...
    // RENDER REQUESTS
    const info = { labels, regionlabels, active };
...
class GlobalMap extends React.Component {
  ...
  CreateMiscIcon(event, type) {
    ...
    socket.emit('updateObject', { type: 'misc_icon', object: packet, key });
  }
  ...
  CreateFob(event) {
    ...
    socket.emit('updateObject', { type: 'fobs', object: packet, key });
  }
