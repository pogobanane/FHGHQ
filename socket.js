    ...
    if (db.logincheck(socket.handshake)) {
      ...
      if (link == process.env.LINK) {
        ...
        if (rank > 0 && rank < 5) {
          ...
          socket.on('addArtyResult', (packet) => {
            //packet
            packet.globalid = getglobalid(socket);
            rank = db.getmembership(account.id, link);
            if (rank > 0 && rank < 4) {
          ...
          socket.on('setDiscordToken', (token) => {
            //packet
            //console.log(packet)
            let globalid = getglobalid(socket);
            rank = db.getmembership(account.id, link);
...
function UpdateStats() {
  ...
  xhr.open(
    'GET',
    'https://api.steampowered.com/ISteamUserStats/GetNumberOfCurrentPlayers/v1/?appid=505460',
    true
  );
      ...
      try {
        ...
        for (let i = 0; i < data.warstats.length - 1; i++) {
          ...
          if (!present) {
            ...
            requestwar.open(
              'GET',
              'http://foxholeglobal.com/service/prevWar/' +
                data.warstats[i].warNumber,
              true
            );
            requestwar.onload = function () {
              if (requestwar.status >= 200 && requestwar.status < 400) {
                try {
                  ...
                  const insertwar = sql.prepare(
                    'INSERT INTO warhistory (warnumber, events, reports, warstats,startpoint) VALUES (@warnumber, @events, @reports, @warstats, @startpoint);'
                  );
                  let e = {
...
function getglobalid(socket) {
  var globalid = socket.handshake.headers.referer.replace(
    process.env.LINK + 'room/',
    ''
  );
