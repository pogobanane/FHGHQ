...
export class ContextDropdown extends React.Component {
    ...
    if (this.state.button != 0) {
      ...
      if (this.state.button >= 16 && this.state.button <= 20) {
        let packetEvent = {
          ...
          packet: { key: key, type: this.state.button },
        };
  ...
  GetItem(i) {
    ...
    return (
      <p
        className='dropdown-item dropdown_item_context-menu'
        onMouseEnter={() => this.HoverIcon(i)}
        onMouseLeave={() => this.HoverIcon(0)}
        key={'contextdropdown' + item.name}
      >
        <img className='dropdown_image' src={item.url} />
        {item.name}
      </p>
    );
  }
