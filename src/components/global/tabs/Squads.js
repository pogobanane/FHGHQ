...
const repo =
  'https://raw.githubusercontent.com/the-fellowship-of-the-warapi/Assets/master/Item Icons/Vehicles/';
...
const vehicleArray = [
  {
    name: 'Truck',
    url: [repo + 'TruckColonial.png', repo + 'TruckWarden.png'],
    roles: ['Driver', 'Passenger'],
  },
  {
    name: 'LUV',
    url: [repo + 'LUV_Colonial.png', repo + 'LUV_Warden.png'],
    roles: ['Driver', 'Passenger 1', 'Passenger 2', 'Passenger 3'],
  },
  {
    name: 'Armored Car',
    url: [repo + 'ArmoredCarColonial.png', repo + 'ArmoredCarWarden.png'],
    roles: ['Driver', 'Gunner'],
  },
  {
    name: 'Half Track',
    url: [repo + 'HalfTrackColonial.png', repo + 'HalfTrackWarden.png'],
    roles: ['Driver', 'Machine Gunner', 'Medic'],
  },
  {
    name: 'Light Tank',
    url: [repo + 'LightTankColonial.png', repo + 'LightTankWarden.png'],
    roles: ['Driver', 'Gunner', 'Commander'],
  },
  {
    name: 'Battle Tank',
    url: [repo + 'BattleTankColonial.png', repo + 'BattleTankWarden.png'],
    roles: [
      'Driver',
      'Machine gunner',
      'Turret gunner',
  ...
  {
    name: 'Gunboat',
    url: [repo + 'GunboatColonial.png', repo + 'GunboatWarden.png'],
    roles: ['Driver', 'Machine Gunner', 'Artillery Gunner', 'Commander'],
  },
  {
    name: 'Field Artillery',
    url: [
      repo + 'FieldArtilleryColonial.png',
      repo + 'FieldArtilleryWarden.png',
    ],
    roles: ['Driver', 'Gunner'],
  },
  // {name:"",url:["",""],roles:["",""]},
...
////////////////////////////////////////////
class SquadsCore extends React.Component {
  ...
  render() {
    ...
    if (this.props.squads.squads != undefined) {
      ...
      squadlist = this.props.squads.squads.map((obj, index) => (
        <Squad
          key={'squad' + obj.users[0]}
          users={this.props.users}
          index={index}
          squads={this.props.squads.squads}
        />
      ));
    }
    ...
    return (
      <React.Fragment>
        <button id='squads_create_btn' onClick={this.CreateSquad}>
          Create squad
        </button>
...
class Squad extends React.Component {
  ...
  GetUser(index) {
    try {
      ...
      let name = (
        <p className='squad_user_name'>
          <img
            className='card_ambush_removeimage'
            src={U.roleicons[user.role[0]].url}
          />
          {user.name}
        </p>
      );
      return (
        <tr className='squad_user_row' key={'squaduser' + user.id}>
          <td className='squad_user_avatar_cell'>
            <img className='squad_user_avatar' src={user.avatar} />
          </td>
          <td className='squad_user_name_cell'>{name}</td>
        </tr>
      );
    } catch (err) {}
  }
  CheckIfMine() {
    //CODE 0:NOT IN SQUAD //CODE 1:SQUAD MEMBER //CODE 2:SQUAD LEADER
    let squad = this.props.squads[this.props.index];
  ...
  render() {
    ...
    if (rolecode == 2) {
      squadname = (
        <span className='squad_name_span'>
          <input
            type='text'
            maxLength='16'
            className='squad_name_input'
    ...
    return (
      <div className='squads_squadcontainer'>
        <div className='squad_name_row'>
          <img
            className='squads_squadcontainer_descIcon'
            src={U.squadnumbers[this.props.index]}
          />{' '}
          {squadname}
        </div>
        <table className='squads_squadtable'>
          <tbody>{userlist}</tbody>
        </table>
        <button
          className='squads_btn_leavejoin'
          onClick={() => this.LeaveJoin(rolecode)}
        >
          {buttonlabel}
        </button>
      </div>
...
class RoleList extends React.Component {
  ...
  GetRole(index) {
    return (
      <td className='popover_role_cell'>
        <button
          className='popover_submit_btn'
          onClick={() => this.ChangeRole(index, 0)}
        >
          <img className='popover_submit_img' src={U.roleicons[index].url} />
          {U.roleicons[index].name}
        </button>
      </td>
    );
  }
  ...
  render() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      for (var j = 0; j < 4; j++) {
        if (rolecounter[i + 1 + j * 3] != undefined) {
          row.push(
            <td key={'rolecounter' + i + '|' + j}>
              {this.GetImage(i + 1 + j * 3, rolecounter[i + 1 + j * 3])}
            </td>
          );
        } else {
    ...
    return (
      <React.Fragment>
        <div id='squads_rolecounter_table'>
          <table>
            <thead className='squads_roleselect_header'>
              <tr>
                <th>
                  <p>Logi {rolecounter2[1]}</p>
                </th>
                <th>
                  <p>Combat {rolecounter2[2]}</p>
                </th>
              </tr>
            </thead>
          </table>
        ...
        <table id='squads_roleselect_table'>
          <thead className='squads_roleselect_header'>
            <tr>
              <th>Select role</th>
              <th>
...
///////////////////////////////////////////////
class ArmyComp extends React.Component {
  ////Component generation
  constructor(props) {
    super(props);
    this.state = {
  ...
  GetVehicleTable() {
    ...
    while (true) {
      ...
      for (var j = 0; j < vehicleArray.length; j++) {
        ...
        if (props[type] != undefined) {
          if (props[type][index] != undefined) {
            ...
            item = (
              <React.Fragment>
                <img
                  className='squads_armycomp_vehicleimg squads_armycomp_vehicle_categoryimg'
                  src={vehicleArray[j].url[this.props.faction]}
                  onClick={() => this.SelectCategory(indexj)}
                />
                <p className='squads_armycomp_crewamount_p'>
                  {crew}/{vehicleArray[j].roles.length}
                </p>
              </React.Fragment>
            );
          }
        ...
        row.push(
          <td
            className='squads_armycomp_itemcell'
            key={'armycompvtablecell' + index + '|' + indexj}
          >
            {item}
          </td>
        );
  ...
  render() {
    ...
    for (var i = 0; i < vehicleArray.length; i++) {
      ...
      if (this.state.selectedCategory == index) {
        classname =
          'squads_armycomp_category_col squads_armycomp_category_col_selected';
        AddBtn = (
          <button
            id='squads_armycomp_category_addbtn'
            className='btn'
            onClick={() => this.AddVehicle(index)}
          ></button>
        );
      }
      buttonlist.push(
        <th className={classname} key={'squadvtablebutton' + i}>
          <button
            className='squads_armycomp_vehiclebtn'
            onClick={
              () => this.SelectCategory(index) /*this.AddVehicle(index)*/
            }
          >
            <img
              className='squads_armycomp_vehicleimg '
              src={vehicleArray[i].url[this.props.faction]}
            />
          </button>
          {AddBtn}
        </th>
    ...
    if (this.state.selectedCategory != 100) {
      let category =
        this.props.squads.vehicles[
          vehicleArray[this.state.selectedCategory].name
        ];
    ...
    return (
      <div id='squad_armycomp_container' className='row'>
        <table id='squad_armycomp_table' className='col'>
          <thead>
            <tr id='squads_armycomp_vehicle_list'>{buttonlist}</tr>
          </thead>
          <tbody>{rowarray}</tbody>
        </table>

        <div className='col squads_armycomp_vehicle_row'>{vehiclelist}</div>
      </div>
    );
  }
}
///////////////////////////////////////////////
class ArmyCompVehicle extends React.Component {
  ////Component generation
  constructor(props) {
    super(props);
    this.RemoveVehicle = this.RemoveVehicle.bind(this);
  ...
  render() {
    ...
    for (var i = 0; i < vehicleStatic.roles.length; i++) {
      ...
      if (vehicle[role] == undefined || vehicle[role] == '') {
        item = (
          <button
            onClick={() => this.JoinRole(role)}
            className='btn squads_armycomp_card_joinbtn'
          >
            Join
          </button>
        );
        ...
        item = (
          <p className='squads_armycomp_card_p'>
      ...
      rolelist.push(
        <tr>
          <td className='squads_armycomp_card_td1'>
            <p className='squads_armycomp_card_p'>{role}</p>
          </td>
          <td className='squads_armycomp_card_td2'>{item}</td>
        </tr>
      );
    }
    return (
      <div className='squads_armycomp_card'>
        <div className='row squads_armycomp_card_row'>
          <div className='col squads_armycomp_card_img_col'>
            <img
              className='squads_armycomp_card_img'
              src={vehicleStatic.url[this.props.faction]}
            />
          </div>
        ...
        <button
          className='squads_armycomp_card_remove_btn'
...
const squadicons = [
  ...
  'https://cdn.glitch.com/6393f3fd-16a7-4641-ae3d-994f8e7cea4e%2FMapIconVehicle.png?1547280031111',
];
