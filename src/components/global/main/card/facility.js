...
class Facility_ extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    let old = this.props.storeObj;
    let newobj = nextProps.storeObj;
    if (this.props.collateral != undefined) {
      if (
        this.props.selected[this.props.type] !=
        nextProps.selected[this.props.type]
      ) {
        return true;
      }
      if (
        this.props[this.props.type] == undefined &&
        nextProps[this.props.type] == undefined
      ) {
        return false;
      }
      if (
        JSON.stringify(this.props[this.props.type]) !=
        JSON.stringify(nextProps[this.props.type])
      ) {
        return true;
      }
    }
    if (this.props.type == 'refinery') {
      ...
      let a = {
        smallarms: old.smallarms,
        heavyarms: old.heavyarms,
        utility: old.utility,
        supplies: old.supplies,
        medical: old.medical,
      };
      let b = {
        smallarms: newobj.smallarms,
        heavyarms: newobj.heavyarms,
        utility: newobj.utility,
        supplies: newobj.supplies,
        medical: newobj.medical,
      };
      if (JSON.stringify(a) != JSON.stringify(b)) {
        return true;
      }
  ...
  render() {
      ...
      if (obj[facility.name] == facilitylimit) {
        ...
        fillbtn = (
          <button
            ...
            onClick={() => card.handleChangeFacility(facility, 0)}
          >
            Empty
          </button>
        );
        ...
        input = (
          <input
            ...
            min='0'
            max={facilitylimit}
            value={obj[facility.name]}
            onChange={(event) =>
              card.handleChangeFacility(facility, event.target.value)
            }
          />
        );
        fillbtn = (
          <button
            type='button'
            className='btn cardbtn useronly'
            onClick={() => card.handleChangeFacility(facility, facilitylimit)}
          >
            Fill
          </button>
        );
        pbar = (
          <div
            ...
            style={{ width: (obj[facility.name] / facilitylimit) * 100 + '%' }}
          >
            {obj[facility.name] + '/' + facilitylimit}
          </div>
        );
      }
      return (
        <tr style={{ height: 26 }}>
          <td style={{ width: 25 }}>
            <img className='towncardimage' src={facility.src} />{' '}
          </td>
    ...
    let rows = facilitytypes.array[type].map((facility) =>
      RenderLine(facility)
    );
    return (
      <React.Fragment>
