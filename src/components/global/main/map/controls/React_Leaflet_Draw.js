...
class RLDComp_ extends React.Component {
  ...
  handleCreate(e) {
    ...
    switch (e.layerType) {
      case 'circle':
        packet = {
          position: [e.layer._latlng.lat, e.layer._latlng.lng],
  ...
  handleEdit(e) {
    ...
    for (var prop in e.layers._layers) {
      ...
      switch (obj.options.type) {
        case 'circle':
          packets.push({
            position: [obj._latlng.lat, obj._latlng.lng],
            radius: obj._mRadius,
        ...
        case 'polygon':
          packets.push({
            position: this.convertArray(obj._latlngs[0]),
            type: obj.options.type,
          });
          break;
        case 'polyline':
          packets.push({
            position: this.convertArray(obj._latlngs),
            type: obj.options.type,
          });
          break;
      }
      ...
      <L.FeatureGroup>
        ...
        {this.state.draw ? (
          <LeafletControl.default
            position='topleft'
            className='leaflet_control_pan'
          >
            <span>Style</span>{' '}
            <select value={this.state.style} onChange={this.handleChangeStyle}>
...
class Arrow extends React.Component {
  ...
  render() {
    ...
    let icon = (
      <L.Marker
        position={obj.position[obj.position.length - 1]}
        icon={arrow_icon}
      />
    );
    return (
