...
class FobPanel_ extends React.Component {
  ...
  render() {
    ...
    let classes = [
      'fob_radio fob_radio_neutral ',
      'fob_radio fob_radio_colonial ',
      'fob_radio fob_radio_warden ',
    ];
    for (var i = 0; i < 3; i++) {
      if (side == i) {
      ...
      <React.Fragment>
        ...
        <div className='fob_card_container'>
          <button
            className={classes[0]}
            disabled={side == 0}
            onClick={() => this.handleChangeSide(0)}
          />
          <button
            className={classes[1]}
            disabled={side == 1}
            onClick={() => this.handleChangeSide(1)}
          />
          <button
            className={classes[2]}
            disabled={side == 2}
