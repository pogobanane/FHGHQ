...
function GridLines_(props) {
  ...
  let k = RegionImages.k,
    w = RegionImages.w,
    o = RegionImages.regionlist[props.index].center;
  let gridlines = [];

  for (
    let xcoord = o[1] - w / 2;
    xcoord < o[1] + w / 2;
    xcoord += RegionImages.ratio * 125
  ) {
    gridlines.push(
      <L.Polyline
        key={'x' + xcoord}
    ...
    for (let i = 0; i * RegionImages.ratio * 125 < w; i++) {
      for (let j = 0; j * RegionImages.ratio * 125 < k; j++) {
        ...
        var myIcon = NativeL.divIcon({
          html: "<span class='gridtext'>" + horgrid + '' + (j + 1) + '</span>',
        });
        ...
        gridlines.push(
          <L.Marker
            key={i + '|' + j}
            position={[
              o[0] + k / 2 - j * RegionImages.ratio * 125,
              o[1] - w / 2 + i * RegionImages.ratio * 125,
            ]}
            icon={myIcon}
            className='map_grid_divicon'
          />
        );
      }
    }
